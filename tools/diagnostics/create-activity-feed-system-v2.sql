...
-- VERSION 2: With type casting for family_id mismatches
...
-- CAST: profiles.family_id (UUID) to TEXT to match activity_feed.family_id (TEXT)
...
-- Policy: Family members can view reactions on their family's activities
...
-- Policy: Family members can view comments on their family's activities
-- CAST: profiles.family_id (UUID) to TEXT
...
-- Policy: Users can delete their own comments, parents can delete any
-- CAST: profiles.family_id (UUID) to TEXT
...
-- ============================================================================
CREATE OR REPLACE VIEW activity_feed_with_stats AS
SELECT 
  af.*,
...
BEGIN
  ...
  RAISE NOTICE '✅ Created tables: activity_feed, activity_reactions, activity_comments';
  RAISE NOTICE '🔒 RLS policies configured (with UUID→TEXT casts for family_id)';
  RAISE NOTICE '⚡ Triggers created for automatic activity logging';
  RAISE NOTICE '📊 Created view: activity_feed_with_stats';
  RAISE NOTICE '';
  RAISE NOTICE '⚠️  NOTE: Schema has mixed types (families.id=TEXT, profiles.family_id=UUID)';
  RAISE NOTICE '    This script uses ::text casts in all comparisons.';
  RAISE NOTICE '';
  ...
  RAISE NOTICE '1. Build /activity-feed page to display activities';
  RAISE NOTICE '2. Add reaction buttons (like, love, celebrate)';
  RAISE NOTICE '3. Implement real-time updates with Supabase subscriptions';
END $$;
