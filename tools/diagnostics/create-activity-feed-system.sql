...
CREATE POLICY "Family members view reactions"
  ...
  USING (
    EXISTS (
      ...
      JOIN profiles p ON p.family_id = af.family_id
...
CREATE POLICY "Users delete own comments or parents delete any"
  ...
  USING (
    ...
    OR EXISTS (
      SELECT 1 FROM profiles
      JOIN activity_feed af ON af.family_id = profiles.family_id
      WHERE profiles.id = auth.uid()
      AND profiles.role = 'parent'
...
BEGIN
  ...
  IF NEW.completed = true AND (OLD.completed IS NULL OR OLD.completed = false) THEN
    ...
    SELECT family_id, full_name INTO v_family_id, v_user_name
    FROM profiles
