-- ============================================================================
-- Fix Notification Triggers: Use Correct Type Values
-- ============================================================================
-- The notifications table type constraint allows:
-- 'info', 'success', 'warning', 'error', 'task', 'reward', 'reward_suggestion'
-- 
-- This fixes all triggers to use 'task' for task-related notifications.
-- ============================================================================

-- ============================================================================
-- Fix: notify_help_requested - Use type='task'
-- ============================================================================
...
BEGIN
  ...
  IF NEW.help_requested = TRUE AND (OLD.help_requested IS NULL OR OLD.help_requested = FALSE) THEN
    ...
    LOOP
      ...
      VALUES (
        v_parent_id,
        'task',  -- Changed from 'help_request'
        'Help Requested',
...
-- ============================================================================
-- Fix: notify_task_approved - Use type='task'
-- ============================================================================
...
BEGIN
  ...
  IF NEW.approved = true AND (OLD.approved IS NULL OR OLD.approved = false) THEN
    ...
    IF NEW.assigned_to IS NOT NULL THEN
      ...
      VALUES (
        NEW.assigned_to,
        'task',  -- Changed from 'task_approved'
        'Task Approved!',
...
-- ============================================================================
-- Fix: notify_task_assigned - Use type='task'
-- ============================================================================
...
BEGIN
  ...
  IF NEW.assigned_to IS NOT NULL AND (OLD.assigned_to IS NULL OR OLD.assigned_to != NEW.assigned_to) THEN
    ...
    VALUES (
      NEW.assigned_to,
      'task',  -- Changed from 'task_assigned'
      'New Task Assigned',
...
-- ============================================================================
-- Fix: notify_task_completed - Use type='task'
-- ============================================================================
...
BEGIN
  ...
  IF NEW.completed = true AND (OLD.completed IS NULL OR OLD.completed = false) THEN
    ...
    LOOP
      ...
      VALUES (
        v_parent_id,
        'task',  -- Changed from 'task_completed'
        'Task Completed',
...
BEGIN
  RAISE NOTICE '';
  RAISE NOTICE '====================================';
  RAISE NOTICE '✅✅✅ NOTIFICATION TYPE FIXED ✅✅✅';
  RAISE NOTICE '====================================';
  RAISE NOTICE '';
  RAISE NOTICE 'Fixed all 4 trigger functions to use type="task":';
  ...
  RAISE NOTICE '';
  RAISE NOTICE 'All now use type="task" (allowed by constraint)';
  RAISE NOTICE '';
  RAISE NOTICE '====================================';
  RAISE NOTICE 'TEST NOW - Should finally work!';
  RAISE NOTICE '====================================';
END $$;
